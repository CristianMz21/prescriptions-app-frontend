...
describe('prescriptionFilterSchema', () => {
  ...
  it('rejects invalid date format', () => {
    const result = prescriptionFilterSchema.safeParse({ fromDate: '01-01-2026' })
    expect(result.success).toBe(false)
  })

  it('rejects invalid UUID for patientId', () => {
    const result = prescriptionFilterSchema.safeParse({ patientId: 'not-a-uuid' })
    expect(result.success).toBe(false)
  })

  it('rejects invalid sortBy value', () => {
    const result = prescriptionFilterSchema.safeParse({ sortBy: 'invalidField' })
    expect(result.success).toBe(false)
  })
  ...
  it('accepts valid hasNotes boolean', () => {
    expect(prescriptionFilterSchema.safeParse({ hasNotes: true }).success).toBe(true)
    expect(prescriptionFilterSchema.safeParse({ hasNotes: false }).success).toBe(true)
  })
})
