...
const SPEC_PATH = process.env.OPENAPI_PATH ?? join(process.cwd(), '..', '..', 'backend', 'openapi.json')
...
async function main() {
  ...
  try {
    ...
    console.error(`Failed to reach backend at ${API_URL}/docs-json:`, err.message ?? err)
    process.exit(2)
  }
  ...
  console.error('  Run `pnpm run api:refresh` (or your backend export script) to sync.')
