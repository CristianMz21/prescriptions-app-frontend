...
test.describe('Patient prescription flows', () => {
  test('list renders glass cards (not the doctor table)', async ({ page, loginAs }) => {
  ...
  test('clicking View Details opens the detail page', async ({ page, loginAs }) => {
    await loginAs('patient')
    const firstCard = page.getByTestId('prescription-card').first()
  ...
  test('back-link returns to /patient/prescriptions', async ({ page, loginAs }) => {
    await loginAs('patient')
    await page.getByTestId('prescription-card').first().getByRole('link', { name: /view details/i }).click()
  ...
  test('PDF download URL points at the backend and returns application/pdf', async ({
    ...
    expect(accessToken, 'patient access token cookie should be set').toBeDefined()
  ...
  test('a consumed RX detail does not show the consume button', async ({
    ...
    const consumeRes = await apiRequest.patch(`/prescriptions/${fresh.id}/consume`, {
