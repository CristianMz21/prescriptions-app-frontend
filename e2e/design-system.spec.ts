...
test.describe('RX-OS design-system invariants', () => {
    ...
    test(`${role} pages keep Material Symbols + tabular nums + glass surfaces`, async ({
      ...
      for (const path of paths) {
        ...
        if ((await icons.count()) > 0) {
          const family = await icons.first().evaluate((el) => getComputedStyle(el).fontFamily)
          expect(family).toMatch(/Material Symbols Outlined/i)
        }
  ...
  test('status badges are monochrome (no green/red hues)', async ({ loginAs, page }) => {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      expect(greenDominant, `badge ${i} is green-dominant: ${color}`).toBe(false)
