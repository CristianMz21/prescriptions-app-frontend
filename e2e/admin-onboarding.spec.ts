...
test.describe('Admin onboarding (P1 — usersControllerCreate)', () => {
  test('admin sees Users + Doctors + All Prescriptions in nav', async ({ loginAs, page }) => {
    await loginAs('admin')
  ...
  test('admin can open the New User form', async ({ loginAs, page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Onboard new user/i })).toBeVisible()
  })
  ...
  test('admin can browse all prescriptions with filters', async ({ loginAs, page }) => {
    await loginAs('admin')
